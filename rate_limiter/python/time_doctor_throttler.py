...
class TimeDoctorThrottler(_TimeDoctorThrottlerDefaultsBase, RequestThrottler):
    ...
    def _get_api_key(self):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS td_token (
                token TEXT,
                created_at TEXT
            )
        """)
        ...
        cursor.execute("SELECT token, created_at FROM td_token ORDER BY created_at DESC LIMIT 1")
        row = cursor.fetchone()
        if row:
            token, created_at_str = row
            created_at = datetime.fromisoformat(created_at_str) 
            if datetime.utcnow() - created_at < timedelta(days=150):
                conn.close()
                return token
        ...
        cursor.execute("DELETE FROM td_token")
        cursor.execute("INSERT INTO td_token (token, created_at) VALUES (?, ?)", (token, datetime.utcnow().isoformat()))
        conn.commit()
        conn.close()
        return token
